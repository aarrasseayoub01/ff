...
const Zone = (props) => {
    ...
    try {
      if (window.confirm("Etes-vous sûr que vous voulez Supprimer ?")) {
        ...
        const res = await axios.get(
          "http://localhost:5000/api/equipement/relatedTo/" + props.zoneId
        );
        await axios.put("http://localhost:5000/api/zone/d/" + zone._id, {
          responsable: zone.responsable,
          equipe: zone.equipe,
        });
        for (let i = 0; i < res.data.length; i++) {
          await axios.put(
            "http://localhost:5000/api/equipement/" + res.data[i]._id,
            { ...res.data[i], zone: "" }
          );
        }
        navigate("../zones");
  ...
  const deleteThisResponsable = async (id) => {
    await axios.put("http://localhost:5000/api/zone/" + props.zoneId, {
      ...zone,
      responsable: "",
    });
  ...
  const deleteThisEquipe = async (id) => {
    ...
    await axios.put("http://localhost:5000/api/zone/" + props.zoneId, {
      ...zone,
      equipe: deletedEquipe,
    });
    setZone({ ...zone, equipe: deletedEquipe });
  };
  const handleDownload = async (e) => {
    await axios.get(
      "http://localhost:5000/api/download/" + zone.flux[e.target.id]
    );
  };
  ...
  useEffect(() => {
    ...
    const fetchZone = async () => {
      const res = await axios.get(
        "http://localhost:5000/api/zone/" + props.zoneId
      );
      setZone(res.data);
    };
    fetchZone();
      ...
      if (allIds.includes(x)) {
        return (
          ...
          >
            <b>
              {index === 0 ? "  " : " - "}{" "}
              {persons[allIds.indexOf(x)].nom +
                " " +
                persons[allIds.indexOf(x)].prenom}
            </b>
          </Link>
        );
      } else {
        return (
          <Tooltip title="Double Click pour supprimer ce personnel Définitivement !">
            <b
              ...
              onDoubleClick={() => deleteThisEquipe(x)}
            >
              Le personnel qui a été Affecté est Supprimé !
            </b>
          </Tooltip>
        );
      }
    ...
    <main className="container">
      <div className="container">
        ...
        <div className="col-4 small d-flex justify-content-center align-items-center">
          <Button href="../zones" className="col-2 small mx-2">
            <AiFillCaretUp />
            Liste des Zones
          </Button>
        </div>
        <div className="text-center">
          <h1 className="text-center">
            {zone.type === "zone" ? "Zone" : "Bâtiment"} : {zone.code}
          </h1>
        </div>
          ...
          >
              ...
              {persons !== undefined && zone.responsable !== "" ? (
                allIds.includes(zone.responsable) ? (
                  <Link
                    ...
                    to={
                      "/personnel/" +
                      persons[allIds.indexOf(zone.responsable)]._id
                    }
                  >
                    <b>
                      {persons[allIds.indexOf(zone.responsable)].nom +
                        " " +
                        persons[allIds.indexOf(zone.responsable)].prenom}
                    </b>
                  </Link>
                ) : (
                  <Tooltip title="Double Click pour supprimer ce personnel Définitivement !">
                    <b
                      ...
                      onDoubleClick={deleteThisResponsable}
                    >
                      Le personnel qui a été Affecté est Supprimé !
                    </b>
                  </Tooltip>
            ...
            <div className="d-block w-100 m-1">
              ...
              {transformedPersonnel.length !== 0 ? (
                ...
                <b>
                  Le(s) personnel(s) qui a(ont) été Affecté est(sont)
                  Supprimé(s) !
                </b>
              )}
            </div>
        ...
        <div className="container text-center">
          <Button className="btn text-danger" onClick={deleteZone}>
            Supprimer
          </Button>
        </div>
      </div>
