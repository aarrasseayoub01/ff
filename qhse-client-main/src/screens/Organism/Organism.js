...
export default function Organism(props) {
  ...
  const submitEditOrg = async () => {
    if (isValid) {
      try {
        await axios.put(
          "http://localhost:5000/api/organism/" + props.orgId,
          editValues
        );
      } catch (err) {
        console.log(err);
  ...
  useEffect(() => {
    ...
    const fetchProds = async () => {
      const res = await axios.get(
        "http://localhost:5000/api/product/a/" + org._id
      );
      setProds(res.data);
    };
    fetchProds();
    ...
    const fetchFours = async () => {
      const res = await axios.get(
        "http://localhost:5000/api/fournisseur/a/" + user._id
      );
      setFours(res.data);
    };
    fetchFours();
    const fetchPersons = async () => {
      if (user) {
        const res = await axios.get(
          "http://localhost:5000/api/personnel/a/" + user._id
        );
        setPersons(res.data);
      }
    };
    ...
    const fetchEquips = async () => {
      const res = await axios.get(
        "http://localhost:5000/api/equipement/all/" + user._id
      );
      setEquips(res.data);
    };
    fetchEquips();
  ...
  const fournisseurs =
    ...
    fours.slice(0, 3).map((x, i) => {
      return (
        <Fournisseurs num={i + 1} key={x._id} fourId={x} dashBoard={true} />
      );
    });
  const personnels =
    persons &&
    persons.slice(0, 3).map((x, i) => {
      return (
        <Personnels
          num={i + 1}
          key={x._id}
  ...
  const equipements =
    equipementsFilter !== undefined &&
    equipementsFilter.slice(0, 3).map((x, i) => {
      return (
        <Equipement num={i + 1} key={x._id} equipId={x} dashBoard={true} />
      );
    });
  // const prods1 =
  //   prods !== undefined &&
  ...
  //         src={"http://localhost:5000/images/" + prod.photos[0]}
  //       />
  //     </Carousel.Slide>;
  //   });
    ...
    return !isEdit ? (
      <div className="grid-container">
        <div className="container bg-white1 grid-item1 center">
          <h1 className="text-prime pb-5 ">
            {!lastValues.name ? editValues.name : lastValues.name}
          </h1>
          <div className="m-2">
            Sites :{" "}
            <b>
              {!lastValues.site_num ? editValues.site_num : lastValues.site_num}
            </b>
          </div>
          <div className=" m-2">
            Date de création :{" "}
            <b>
              {!lastValues.creation_date
                ? formatDate(editValues.creation_date)
                : formatDate(lastValues.creation_date)}
            </b>
          </div>
          <div className=" m-2">
            Domaine :{" "}
            <b>
              {!lastValues.domaines ? editValues.domaines : lastValues.domaines}
            </b>
          </div>
          <div className=" m-2">
            Téléphone :{" "}
            <b>{!lastValues.tel ? editValues.tel : lastValues.tel}</b>
          </div>
          <div className=" m-2">
            Adresse :{" "}
            <b>
              {!lastValues.Adresse ? editValues.Adresse : lastValues.Adresse}
            </b>
          </div>
          <div className=" m-2">
            Localisation :{" "}
            <b>{!lastValues.Carte ? editValues.Carte : lastValues.Carte}</b>
          </div>
          <div className="mt-5">
            <Button className="bg-prime" onClick={editOrg}>
              Modifier
            </Button>
            <Button className="bg-danger" onClick={delOrg}>
              Supprimer
            </Button>
          </div>
        </div>
        <div className="container grid-item2 grid-container2">
          <div className="item1 bg-white1 grid-container3">
            <div className="itemA text-prime">
              <h2 onClick={() => navigate("/products")}>Les produits:</h2>
            </div>
            <div className="itemB">
              <Carousel
                sx={{ maxWidth: 320 }}
              ...
              >
                {prods[0] !== undefined && (
                  <Carousel.Slide>
                    ...
                    <Image
                      style={{ cursor: "pointer" }}
                      onClick={() => navigate("/product/" + prods[0]._id)}
                      src={"http://localhost:5000/images/" + prods[0].photos[0]}
                    />
                  </Carousel.Slide>
                )}
                {prods[1] !== undefined && (
                  <Carousel.Slide>
                    ...
                    <Image
                      style={{ cursor: "pointer" }}
                      onClick={() => navigate("/product/" + prods[1]._id)}
                      src={"http://localhost:5000/images/" + prods[1].photos[0]}
                    />
                  </Carousel.Slide>
                )}
                {prods[2] !== undefined && (
                  <Carousel.Slide>
                    ...
                    <Image
                      style={{ cursor: "pointer" }}
                      onClick={() => navigate("/product/" + prods[2]._id)}
                      src={"http://localhost:5000/images/" + prods[2].photos[0]}
                    />
                  </Carousel.Slide>
                )}
              </Carousel>
              ...
              >
                {raws[0] !== undefined && (
                  <Carousel.Slide>
                    ...
                    <Image
                      style={{ cursor: "pointer" }}
                      onClick={() => navigate("/raw/" + raws[0].name)}
                      src={"http://localhost:5000/images/" + raws[0].photos[0]}
                    />
                  </Carousel.Slide>
                )}
                {raws[1] !== undefined && (
                  <Carousel.Slide>
                    ...
                    <Image
                      style={{ cursor: "pointer" }}
                      onClick={() => navigate("/raw/" + raws[1].name)}
                      src={"http://localhost:5000/images/" + raws[1].photos[0]}
                    />
                  </Carousel.Slide>
                )}
                {raws[2] !== undefined && (
                  <Carousel.Slide>
                    ...
                    <Image
                      style={{ cursor: "pointer" }}
                      onClick={() => navigate("/raw/" + raws[2].name)}
                      src={"http://localhost:5000/images/" + raws[2].photos[0]}
                    />
                  </Carousel.Slide>
                )}
              </Carousel>
          ...
          <div className="item3 bg-white1 grid-container3">
            <div className="itemA text-prime">
              <h2 onClick={() => navigate("/fournisseurs")}>
                Les fournisseurs:
              </h2>
            </div>
            <div className="itemB">
          ...
          <div className="item6 bg-white1 grid-container3">
            <div className="itemA text-prime">
              <h2 onClick={() => navigate("../../equipements")}>
                Les equipements:
              </h2>
            </div>
            <div className="itemB">
      ...
      <div className="container justify-content-center">
        ...
        <div className="row mediaquery-770">
          <div className="col-12 org-edit-label">
            ...
            <input
              ...
              onChange={(event) =>
                editValuesFunc(event.target.name, event.target.value)
              }
            ></input>
          </div>
          <div className="col-12 org-edit-label">
          ...
          <div className="col-12 org-edit-label">
            ...
            <input
              ...
              type="text"
              placeholder={
                !lastValues.Carte ? editValues.Carte : lastValues.Carte
              }
              onChange={(event) =>
                editValuesFunc(event.target.name, event.target.value)
              }
        ...
        <div className="row p-5">
          <Button
            type="submit"
            className="col-3 btn btn-warning m-1"
            onClick={endEditOrg}
          >
            Annuler
          </Button>
          <Button
            type="submit"
            className="col-3 btn btn-success m-1"
