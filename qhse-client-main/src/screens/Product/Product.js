...
const Product = (props) => {
  ...
  const handleEdit = async () => {
    ...
    try {
      await axios.put("http://localhost:5000/api/product/" + props.productId, {
        ...editValues,
        editCount: editValues.editCount + 1,
      });
    } catch (err) {
      console.log(err);
    }
  ...
  const handleDelete = async (e) => {
    ...
    try {
      await axios.delete(
        "http://localhost:5000/api/product/" + props.productId
      );
      navigate("/products");
    } catch (err) {
      console.log(err);
  ...
  const handleDownload = async (e) => {
    switch (e.target.id) {
      case "fiche_tech":
        await axios.get(
          "http://localhost:5000/api/download/" + editValues.fiche_technique
        );
        break;
      case "fds":
  ...
  function navigateLeft() {
    const previousProductId =
      orgProducts && orgProducts[thisProductIndex - 1]._id;
  ...
  useEffect(() => {
    ...
    const fetchMyOrg = async () => {
      const res = await axios.get(
        "http://localhost:5000/api/organism/" + org._id
      );
  ...
  const mappedUserEtiquettes =
    ...
    Object.keys(editValues.userEtiquettes).map((key, value) => {
      return (
        <h5 className="p-3">
          {key}: <b>{editValues.userEtiquettes[key]}</b>
        </h5>
      );
    });
  const mappedModifiedUserEtiquettes =
    editValues.userEtiquettes &&
    Object.keys(editValues.userEtiquettes).map((key, value) => {
      return (
      ...
      <AnimatedPage>
        <div className="container">
          <p className="text-center">
            Numéro du produit : {thisProductIndex + 1}
          </p>
          <div className="row">
            <Box sx={{ height: 40 }}>
              <Fade
              ...
              <div className="container">
                {!isEdit ? (
                  <div className="product-scroll">
                    <h5 className="p-3">
                      Shelf Life: <b>{editValues.shifelife}</b>
                    </h5>
                    <h5 className="p-3">
                      Durée de Shelf Life: <b>{editValues.shife_time}</b>
                    </h5>
                    <h5 className="p-3">
                      Fiche Technique:{" "}
                      <b id="fiche_tech" onClick={handleDownload}>
                        {editValues.fiche_technique.slice(13)}
                      </b>
                    </h5>
                    <h5 className="p-3">
                      FDS:{" "}
                      <b id="fds" onClick={handleDownload}>
                        {editValues.fds.slice(13)}
                      </b>
                    </h5>
                    <h5 className="p-3">
                      Emballage: <b>{editValues.emballage}</b>
                    </h5>
                    <h5 className="p-3">
                      Grammage: <b>{editValues.grammage}</b>
                    </h5>
                    <h5 className="p-3">
                      Type de client: <b>{editValues.type_client}</b>
                    </h5>
                    <h5 className="p-3">
                      Date de création: <b>{editValues.creation_date}</b>
                    </h5>
                    <h5 className="p-3">
                      Agrément Sanitaire: <b>{editValues.agrement}</b>
                    </h5>
                    <h5 className="p-3">
                      Autorisation Sanitaire: <b>{editValues.autorisation}</b>
                    </h5>
                    <h5 className="p-3">
                      Site de production: <b>{editValues.site}</b>
                    </h5>
                    <h5 className="p-3">
                      Organisme:{" "}
                      <b>{productOrganism && productOrganism.name}</b>
                    </h5>
                    <h4 className="text-center col-12 col-sm-6 col-md-4 col-lg-4 etiquettes">
                      Etiquettes
                    </h4>
                    <h5 className="p-3">
                      Valeur Energétique: <b>{editValues.energie}</b>
                    </h5>
                    <h5 className="p-3">
                      Protéines: <b>{editValues.proteine}</b>
                    </h5>
                    <h5 className="p-3">
                      Carbohydrates: <b>{editValues.carbs}</b>
                    </h5>
                    <h5 className="p-3">
                      Lipides: <b>{editValues.lipide}</b>
                    </h5>
            ...
            <div className="container text-center products-btn d-flex justify-content-between align-items-center">
              ...
              <div className="d-flex justify-content-center align-items-start">
                <div className="d-flex flex-column justify-content-center align-items-center">
                  ...
                  <span>
                    Compteur de modification : <b>{editValues.editCount}</b>
                  </span>
                </div>
              ...
              {orgProducts && thisProductIndex !== orgProducts.length - 1 ? (
                <FaArrowCircleRight
                  className="pointer"
                  size={20}
                ...
                <FaArrowCircleRight color="gray" size={20} />
              )}
            </div>
          ) : (
            <div className="d-flex justify-content-center align-items-center">
              <Button className="btn-warning m-2" onClick={handleCancel}>
                Annuler
              </Button>
              <Button className="btn-primary m-2" onClick={handleEdit}>
                Enregistrer
              </Button>
            </div>
          )}
