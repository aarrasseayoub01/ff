...
export default function NewRaw() {
  ...
  const submitProduct = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        "http://localhost:5000/api/product/name/" + product.product
      );
      await axios.put("http://localhost:5000/api/product/" + res.data._id, {
        ...res.data,
        raw: [...res.data.raw, product.name],
      });
    } catch (err) {
      console.log(err);
    }
  ...
  function deleteImage(imageId) {
    setPicture((prev) =>
      prev.filter((picture) => prev.indexOf(picture) !== imageId)
    );
    setImagePage((prev) => {
      if (prev !== 1) {
        return prev - 1;
  ...
  return (
    <main className="container">
      <div className="row text-center">
        <h1 className="text-center">Ajouter Matière Propre</h1>
      </div>
      ...
      <div className="d-flex flex-column align-items-center">
        ...
        >
          ...
          <select
            name="product"
            className="form-select col-12 col-sm-6 col-md-4 col-lg-4"
          ...
          <div className="d-flex justify-content-start align-items-start col-12 col-sm-6 col-md-4 col-lg-4">
            ...
            <label
              ...
              for="fiche_tech"
            >
              <AiOutlineCloudUpload style={{ cursor: "pointer" }} size={20} />{" "}
              <input
                hidden
                type="file"
                id="fiche_tech"
        ...
        <hr />
        <div className="text-center col-12 col-sm-6 col-md-4 col-lg-4">
          Etiquettes
        </div>
        <Box
          className="row d-flex justify-content-center"
        ...
        >
          {product.energie === "" ||
          Number.isInteger(+product.energie) !== false ? (
            <TextField
              className="col-12 col-sm-6 col-md-4 col-lg-4"
              id="outlined-name"
          ...
          {product.proteine === "" ||
          Number.isInteger(+product.proteine) !== false ? (
            <TextField
              className="col-12 col-sm-6 col-md-4 col-lg-4"
              id="outlined-name"
              label="Proteines"
          ...
          {product.carbs === "" ||
          Number.isInteger(+product.carbs) !== false ? (
            <TextField
              className="col-12 col-sm-6 col-md-4 col-lg-4"
              id="outlined-name"
              label="Carbohydrates"
          ...
          {product.lipide === "" ||
          Number.isInteger(+product.lipide) !== false ? (
            <TextField
              className="col-12 col-sm-6 col-md-4 col-lg-4"
              id="outlined-name"
              label="Lipides"
        ...
        <Button className="m-3" variant="outlined" onClick={submitProduct}>
          Enregistrer
        </Button>
      </div>
    </main>
