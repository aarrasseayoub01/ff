...
const Personnel = (props) => {
  ...
  const editPersonnel = async () => {
    ...
    if (Object.keys(editValues).every((x) => editValues[x] !== "")) {
      const updatedPersonnel = {
        ...editValues,
        photo: picture[thisProductIndex],
      };
      try {
        await axios.put(
          "http://localhost:5000/api/personnel/" + props.personId,
          updatedPersonnel
        );
      } catch (err) {
        window.alert("Erreur");
  ...
  const thisProductIndex =
    allPersonnel && allPersonnel.findIndex((x) => x._id === props.personId);
  function navigateLeft() {
    const previousProductId =
      allPersonnel && allPersonnel[thisProductIndex - 1]._id;
      ...
      <div className="container d-flex flex-column justify-content-center align-items-center">
          ...
          {allPersonnel ? (
            ...
            <div className="thumbnail">
              <Avatar
                ...
                src={
                  "http://localhost:5000/images/" + picture[thisProductIndex]
                }
              />
              <span>
                <img
                ...
                />
                <br />
                {allPersonnel[thisProductIndex].nom +
                  " " +
                  allPersonnel[thisProductIndex].prenom}
              </span>
            </div>
          ) : (
            <Skeleton
        ...
        <div className="d-flex justify-content-around align-items-center">
          ...
          <div className="d-flex justify-content-center align-items-center m-3">
            <Button className="text-primary m-2" onClick={editPersonnel}>
              Modifier
            </Button>
            <Button className="text-danger m-2" onClick={deletePersonnel}>
              Supprimer
            </Button>
          </div>
          {allPersonnel && thisProductIndex !== allPersonnel.length - 1 ? (
            <FaArrowCircleRight
              className="pointer"
              size={20}
