...
ZoneRouter.get("/b/:orgId", async (req, res) => {
  try {
    const batiments = await Zone.find({
      organism: req.params.orgId,
      type: "batiment",
    });
...
ZoneRouter.get("/includes/:personId", async (req, res) => {
  try {
    const includedZones = await Zone.find({
      $or: [
        { responsable: req.params.personId },
        { equipe: req.params.personId },
      ],
    });
